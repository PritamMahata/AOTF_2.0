          ...
          {!hideActionButtons && (
            <div className="space-y-2 pt-2">              {!showFullDetails && (
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  onClick={() => {
                    if (canApply && !post.hasApplied && post.status !== 'hold' && !post.hasApprovedTeacher) onApply(post.id);
                  }}
                  className="flex-1"
                  disabled={!canApply || post.hasApplied || post.status === 'hold' || post.hasApprovedTeacher}
                  variant={post.hasApprovedTeacher ? "outline" : post.hasApplied ? "secondary" : post.status === 'hold' ? "outline" : "default"}
                >
                  {post.hasApprovedTeacher ? (
                    <>

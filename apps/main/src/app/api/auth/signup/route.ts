...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create new user WITHOUT creating a session
    // User must login on tutorials or jobs app to get a session
    const user = new User({
      email: email.toLowerCase().trim(),
      password,
      name: (name || '').trim(),
      onboardingCompleted: false, // Will be completed on the chosen app
    });
    ...
    // User will be redirected to choose-path page
    const response = NextResponse.json({
      success: true,
      message: 'Account created successfully. Please choose where to continue.',
      user: {
